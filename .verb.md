...
async function main() {
  ...
  await waitForCompletion({ owner, repo, token, run_id: run.run_id });

  console.log(`Workflow triggered with run_id: ${run.run_id}`);
}
...
Triggers a GitHub Actions workflow and returns a `run` object with the `run_id`, `run_url`, and `html_url` of the triggered workflow.
